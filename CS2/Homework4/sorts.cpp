...
bool compareSensorPtr(const Sensor* lhs, const Sensor* rhs)
{
    ...
    // Student reason
    // here we should be passing the pointers as derreferenced because we have to pass the object. in the compareSensor they have pointers that access the objects data member.
    // Hence why we pass the pointer derefferenced.
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    // we use the vector size because NREADINGS is for sensor array fixed size. it is not the size of a vector. that is why we do s.size().
    // the size may vary for the vector.

    // the way this works is that it takes the current position of i and then looks through the elements behind i position.
    // if the element behind position i is less than the position befor it 
...
int main()
{
    ...
    // destruction time.
    {
        ...
        // student reason below

        // we have to use an iterator because an array name already starts from the beginning and at first we we treating this as an array. 
        // raher we need to treat this as a vector and a vector does not have the same property as an array does, so we must specify where it has to 
        // begin and where the vector ends.

        std::sort(Pointer.begin(), Pointer.end(), compareSensorPtr);
        ...
        /*
        using the now sorted vector of pointers.   THIS MEANS MY POINTER VECTOR.
        replace each sensor in the pointer vector with the sensors from the auxSensor in the correct order.
